...
begin
    ...
    if not exists(select id from ur_user where email = lower(_email)) then
        ...
        insert into ur_user(id, email, password, enabled, date_created, date_modified, ur_person_id, credit_balance, credit_debt, ur_role_id)
        values (uuid_generate_v4(), _email, _password, true, now(), now(), _person_id, _credit_balance, _credit_debt, _role_id)
...
select create_user_if_not_exists('customer@emt.io','$2a$12$pKQiFD9LtQBCZwswOCRUZe.Q2yXRsMmfprZf7YkdZ0LKPdSI/5XiG','EMT', 'Customer', '072000000',
                                 find_role_id_by_name('ROLE_CUSTOMER'), array['DAY_1', 'DAY_2', 'DAY_3', 'DAY_4', 'DAY_5', 'DAY_6', 'DAY_7'], 0.0, 0.0);
